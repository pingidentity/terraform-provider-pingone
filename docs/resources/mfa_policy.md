...
- `new_device_notification` (String) A string that defines whether a user should be notified if a new authentication method has been added to their account. Options are `NONE` (this is the default setting for new environments), `EMAIL_THEN_SMS` and `SMS_THEN_EMAIL`. Defaults to `EMAIL_THEN_SMS`.
