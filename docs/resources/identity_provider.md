...
- `linkedin` (Attributes, Deprecated) A single block that specifies options for connectivity to the LinkedIn social identity provider. (see [below for nested schema](#nestedatt--linkedin))
- `linkedin_oidc` (Attributes) A single block that specifies options for connectivity to the LinkedIn social identity provider.  Exactly one of the following must be defined: `facebook`, `google`, `linkedin`, `linkedin_oidc`, `yahoo`, `amazon`, `twitter`, `apple`, `paypal`, `microsoft`, `github`, `openid_connect`, `saml`.  If this object is added or removed, a replacement plan is triggered.  Parameters within the object are subject to their own immutability rules. (see [below for nested schema](#nestedatt--linkedin_oidc))
...
<a id="nestedatt--linkedin_oidc"></a>
### Nested Schema for `linkedin_oidc`

Required:

- `client_id` (String) A string that specifies the application client ID from LinkedInOIDC.
- `client_secret` (String, Sensitive) A string that specifies the application client secret from LinkedInOIDC.
