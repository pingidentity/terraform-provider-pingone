...
func testAccGetRiskPolicyIDs(resourceName string, environmentID, resourceID *string) resource.TestCheckFunc {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Configure
			{
				...
				Check:  testAccGetRiskPolicyIDs(resourceFullName, &environmentID, &resourceID),
			},
			// Replan after removal preconfig
			{
				PreConfig: func() {
					...
					_, err = apiClient.RiskPoliciesApi.DeleteRiskPolicySet(ctx, environmentID, resourceID).Execute()
					if err != nil {
...
func TestAccRiskPolicy_Import(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Errors
			{
				ResourceName: resourceFullName,
				ImportState:  true,
				ExpectError:  regexp.MustCompile(`Invalid import ID specified \(".*"\).  The ID should be in the format "environment_id/risk_policy_id".`),
			},
			{
				ResourceName:  resourceFullName,
				ImportStateId: "/",
