...
func TestAccTrustedEmailAddress_Full(t *testing.T) {
	...
	check := resource.ComposeTestCheckFunc(
		...
		resource.TestMatchResourceAttr(resourceFullName, "email_domain_id", regexp.MustCompile(`^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`)),
...
resource "pingone_trusted_email_address" "%[2]s" {
  ...
  email_domain_id = data.pingone_trusted_email_domain.email_domain.id

  email_address = "%[3]s"
}`, acctest.DomainVerifiedSandboxEnvironment(verifiedDomain), resourceName, emailAddress)
}
