...
func (r *EditorServiceResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	...
	resp.Schema = schema.Schema{
		...
		Description: "Resource to create and manage authorization services for the PingOne Authorize Trust Framework in a PingOne environment.",
		...
		Attributes: map[string]schema.Attribute{
			...
			"version": schema.StringAttribute{ // DONE
...
func (p *editorServiceValueTypeResourceModel) expand() *authorize.AuthorizeEditorDataValueTypeDTO {
	...
	data := authorize.NewAuthorizeEditorDataValueTypeDTO(
		authorize.EnumAuthorizeEditorDataValueTypeDTO(p.Type.ValueString()),
	)
...
func (p *editorServiceResourceModel) toState(apiObject *authorize.CreateService201Response) diag.Diagnostics {
	...
	if v := apiObject.AuthorizeEditorDataServicesHttpServiceDefinitionDTO; v != nil {
		p.Processor, d = editorDataProcessorOkToTF(v.GetProcessorOk())
		diags.Append(d...)
