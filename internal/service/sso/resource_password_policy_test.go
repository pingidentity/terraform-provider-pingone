	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Configure
			{
				...
				Check:  testAccGetPasswordPolicyIDs(resourceFullName, &environmentID, &resourceID),
			},
			// Replan after removal preconfig
			{
				PreConfig: func() {
					...
					_, err = apiClient.PasswordPoliciesApi.DeletePasswordPolicy(ctx, environmentID, resourceID).Execute()
					if err != nil {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Errors
			{
				ResourceName: resourceFullName,
				ImportState:  true,
				ExpectError:  regexp.MustCompile(`Invalid import ID specified \(".*"\).  The ID should be in the format "environment_id/password_policy_id".`),
			},
			{
				ResourceName:  resourceFullName,
				ImportStateId: "/",
