...
func testAccCheckApplicationResourceGrantDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		body, r, err := apiClient.ApplicationsApplicationResourceGrantsApi.ReadOneApplicationGrant(ctx, rs.Primary.Attributes["environment_id"], rs.Primary.Attributes["application_id"], rs.Primary.ID).Execute()
