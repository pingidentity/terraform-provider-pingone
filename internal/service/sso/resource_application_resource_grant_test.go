...
func TestAccApplicationResourceGrant_Import(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Errors
			{
				ResourceName: resourceFullName,
				ImportState:  true,
				ExpectError:  regexp.MustCompile(`Invalid import ID specified \(".*"\).  The ID should be in the format "environment_id/application_id/resource_grant_id".`),
			},
			{
				ResourceName:  resourceFullName,
				ImportStateId: "/",
