...
func APIService_CheckDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "pingone_authorize_api_service" {
			continue
		}
		...
		_, r, err := apiClient.APIServersApi.ReadOneAPIServer(ctx, rs.Primary.Attributes["environment_id"], rs.Primary.ID).Execute()
		...
		return fmt.Errorf("PingOne API Service Instance %s still exists", rs.Primary.ID)
	}

	return nil
}

func APIService_GetIDs(resourceName string, environmentID, resourceID *string) resource.TestCheckFunc {
...
func APIService_RemovalDrift_PreConfig(ctx context.Context, apiClient *authorize.APIClient, t *testing.T, environmentID, apiServiceID string) {
	...
	if err != nil {
		t.Fatalf("Failed to delete API service: %v", err)
	}
}
