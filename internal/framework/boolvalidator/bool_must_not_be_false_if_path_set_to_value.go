...
// MustNotBeFalseIfPathSetToValue creates a validator that prevents a boolean from being false when another attribute matches a value.
// It returns a validator that ensures the current boolean attribute is not false when the specified path attribute
